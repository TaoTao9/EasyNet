...
class ReconstructiveSubNetwork(nn.Module):
    def __init__(self,in_channels=3, out_channels=3, base_width=128,img_size=256,train_model=True):
        ...
        self.mlp4 = nn.Sequential(
            nn.Conv2d(self.hidden_size*3,self.hidden_size, kernel_size=1, stride=1),
            nn.BatchNorm2d(self.hidden_size),
            nn.LeakyReLU(0.2),
    ...
    def forward(self, x, d):
        ...
        mask = self.mlp5(torch.cat((merge1,merge2,merge3,merge_d1,merge_d2,merge_d3), dim=1))
        mask_rgb = self.mlp4(torch.cat((merge1,merge2,merge3), dim=1))
        return output, output_d, mask, mask_rgb,torch.cat((self.avg_pool(merge1),self.avg_pool(merge2),self.avg_pool(merge3)),dim=1),torch.cat((self.avg_pool(merge1),self.avg_pool(merge2),self.avg_pool(merge3),self.avg_pool(merge_d1),self.avg_pool(merge_d2),self.avg_pool(merge_d3)),dim=1)
