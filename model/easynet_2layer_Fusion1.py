...
class ReconstructiveSubNetwork(nn.Module):
    def __init__(self,in_channels=3, out_channels=3, base_width=128,img_size=256,train_model=True):
        ...
        self.mlp3 = nn.Sequential(
            nn.Conv2d(self.hidden_size*2,self.hidden_size, kernel_size=1, stride=1),
            nn.BatchNorm2d(self.hidden_size),
            nn.LeakyReLU(0.2),
        ...
        self.mlp4 = nn.Sequential(
            nn.Conv2d(self.hidden_size*4,self.hidden_size, kernel_size=1, stride=1),
            nn.BatchNorm2d(self.hidden_size),
            nn.LeakyReLU(0.2),
            nn.Conv2d(self.hidden_size,self.hidden_size1, kernel_size=1, stride=1),
            nn.BatchNorm2d(self.hidden_size1),
    ...
    def forward(self, x, d):
        ...
        mask_rgb = self.mlp3(torch.cat((merge1,merge2), dim=1))
        mask = self.mlp4(torch.cat((merge1,merge2,merge_d1,merge_d2), dim=1))
        # return output, output_d, mask, mask_rgb
        # return output, output_d, mask, mask_rgb, self.avg_pool(merge1),self.avg_pool(merge2),self.avg_pool(merge_d1),self.avg_pool(merge_d2)
        return output, output_d, mask, mask_rgb, torch.cat((self.avg_pool(merge1),self.avg_pool(merge2)),dim=1),torch.cat((self.avg_pool(merge1),self.avg_pool(merge2),self.avg_pool(merge_d1),self.avg_pool(merge_d2)),dim=1)
