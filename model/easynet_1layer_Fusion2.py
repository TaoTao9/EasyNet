...
class ReconstructiveSubNetwork(nn.Module):
    def __init__(self,in_channels=3, out_channels=3, base_width=128,img_size=256,train_model=True):
        ...
        self.mlp1 = nn.Sequential(
            nn.Conv2d(base_width*4,self.hidden_size, kernel_size=1, stride=1),
    ...
    def forward(self,d,merge_rgb):
        d1,d5 = self.encoder_d(d)
        
        output_d,dd4 = self.decoder_d(d5)

        mask = self.mlp(torch.cat((merge_rgb,d1,dd4), dim=1))
        return output_d, mask, self.avg_pool(merge_rgb),self.avg_pool(torch.cat((merge_rgb,d1,dd4), dim=1))
